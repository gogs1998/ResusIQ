...
**Step 2:** run → FAIL. **Step 3:** add `isEmergencyActive`, `activeProtocol`, `currentStepIndex`, `activeEvent`, `timerAnchors` to `partialize`; bump persist `version` and add a `migrate` that tolerates their absence. No flush listener is needed: persist writes synchronously on every `set`, so `activeEvent` is already on disk when the page is discarded, and flushing it into `eventHistory` as well would double-record the emergency — once in the archive and again on the resume (as implemented; see the `merge` comments in `appStore.ts`). **Step 4:** PASS. **Step 5:** gate + commit `fix(store): persist the active emergency so a reload mid-emergency resumes`.
